...
export default async function Home(){
  ...
  <nav className="topbar">
    ...
    <div className="nav-actions"><span className={hasLive?'online':'prelaunch'}><i/> {hasLive?'VERIFIED DATA LIVE':'PRE-LAUNCH'}</span><button>EN⌄</button><a className="admin-nav-link" href="/admin">ADMIN</a></div>
  </nav>
  ...
  <header className="hero" id="top">
    ...
    <section className={hasLive?'situation':'situation pending'} id="updates"><div className="situation-head"><span><b>SITUATION DATA</b><small>{latestUpdate?`UPDATED ${new Date(latestUpdate.updated_at).toLocaleString()}`:'AWAITING FIRST VERIFIED UPDATE'}</small></span><em>{hasLive?'● VERIFIED':'○ NOT LIVE'}</em></div><div className="stats"><div><span className="red">♧</span><small>People in Need</small><b>{peopleInNeed.toLocaleString()}</b></div><div><span className="blue">⌖</span><small>Active Requests</small><b>{requestRecords.length}</b></div><div><span className="green">✓</span><small>Help Provided</small><b>{published.filter(r=>r.status==='resolved').length}</b></div><div><span className="orange">♙</span><small>Verified Volunteers</small><b>{volunteers.toLocaleString()}</b></div></div><p><b>Status:</b> {latestUpdate?latestUpdate.description||latestUpdate.title:'No verified incident reports or operational feeds are connected yet.'}</p><span className="report-pending">{hasLive?'Only coordinator-approved records are shown.':'Situation reports will appear after source verification.'}</span></section>
